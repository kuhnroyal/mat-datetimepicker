...
export class MatDatetimepickerCalendarComponent<D> implements AfterContentInit, OnDestroy {
  @HostBinding('attr.role')
  role = 'dialog';
  @HostBinding('tabindex')
  tabindex = 0;
  @HostBinding('class.mat-datetimepicker-calendar')
  classMatDatetimepickerCalendar = true;
  ...
  ariaLabel = 'Use arrow keys to navigate';
  ...
  _AMPM: string;
  _clockView: MatClockViewType = 'hour';
  ...
  _currentView: MatDatetimeCalendarViewType;

  get currentView(): MatDatetimeCalendarViewType {
    return this._currentView;
  }

  set currentView(view: MatDatetimeCalendarViewType) {
  ...
  get _minutesLabel(): string {
    return toDigit(this._adapter.getMinute(this._activeDate));
  }
  ...
  private _prevMonthInSameCol(date: D): D {
    ...
    // of each year.
    let increment;
    if (this._adapter.getMonth(date) <= 4) {
      increment = -5;
    } else if (this._adapter.getMonth(date) >= 7) {
      increment = -7;
    } else {
      increment = -12;
    }
